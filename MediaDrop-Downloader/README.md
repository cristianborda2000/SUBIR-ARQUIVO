...
3. Para baixar arquivos do painel web, preencha tambem `mediaDropUrl`, `adminUser` e `adminPassword`.
4. Coloque `yt-dlp.exe` em `tools/yt-dlp.exe` ou instale `yt-dlp` no PATH.
5. O FFmpeg pode ficar em `tools/ffmpeg.exe`; se o MediaDrop estiver na pasta ao lado, o baixador tambem usa o FFmpeg de `MediaDrop/node_modules/ffmpeg-static`.
6. Execute:
...
Ao abrir, o aplicativo inicia um painel local no navegador com abas para YouTube e arquivos do MediaDrop. Os downloads ficam na pasta `downloads`.
